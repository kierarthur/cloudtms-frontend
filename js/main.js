      ...
      if (isFlip) {
        ...
        // ── PAYE → UMBRELLA special handling ────────────────────────────────
          ...
          try {
            ...
            L('[onSave][FLIP P→U] postFlipUmbPayload', {
              candidateId: idForUpdate,
        ...
        if (originalMethod === 'UMBRELLA' && newMethod === 'PAYE') {
          ...
          L('[onSave][FLIP U→P] starting UMBRELLA→PAYE flow', {
            candidateId: full.id,
            originalMethod,
            newMethod,
          ...
          try {
            ...
            L('[onSave][FLIP U→P] postFlipPayePayload', {
...
async function mountCandidatePayTab() {
  ...
  function prefillUmbrellaBankFields(umb) {
    ...
    // IMPORTANT: no id guard here – we want repeated prefill for the same umbrella after flips
  ...
  if (payMethod === 'UMBRELLA') {
    ...
    if (hadStagedAtEntry) {
      ...
      // No PAYE-staged state → clear and rebuild from DB/blank
      clearBankAndUmbrella();
...
async function openCandidatePayMethodChangeModal(candidate, context = {}) {
  ...
  let preview;
  try {
    L('preview → fetchCandidatePayMethodChangePreview', { candidateId, newMethod });
  ...
  if (!contracts.length) {
    ...
    try {
      L('[NO-CONTRACTS] applyCandidatePayMethodChange →', {
        candidateId,
        newMethod
      });
  ...
  return await new Promise((resolve) => {
    ...
    showModal(
      ...
      async () => {
        ...
        try {
          L('APPLY → applyCandidatePayMethodChange', {
            candidateId,
            newMethod,
            contractIds
          });
